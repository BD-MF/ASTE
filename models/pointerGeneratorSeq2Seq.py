...
from models.decode import Hypothesis
from models.decode import postprocess
...
class Attention(nn.Module):
    ...
    def __init__(self, hidden_dim, use_coverage):
        ...
        self.enc_proj = nn.Linear(
            hidden_dim * 2, hidden_dim * 2, bias=False)   # W_h
        self.dec_proj = nn.Linear(
            hidden_dim, hidden_dim * 2, bias=True)        # W_s, b_attn
...
class AttnDecoder(nn.Module):
    ...
    def forward(self, dec_input, prev_h, prev_c, enc_hidden, enc_pad_mask, coverage):
        ...
        hidden, cell = self.lstm(
            dec_input, (prev_h, prev_c))   # [B x H], [B x H]
        ...
        context_vec = torch.bmm(attn_dist.unsqueeze(
            1), enc_hidden)     # [B x 1 x 2H]
        context_vec = torch.sum(
            context_vec, dim=1)                     # [B x 2H]
...
class PointerGenerator(nn.Module):
    ...
    def __init__(self, embedding_matrix, config, vocab):
        ...
        # torch.tensor(embedding_matrix, dtype=torch.float).to(config.device)

        emb_vecs = self.embedding_matrix
        ...
        self.decoder = AttnDecoder(input_dim=embed_dim,
                                   ...
                                   use_coverage=config.is_coverage)
        ...
        self.min_dec_steps = config.max_dec_steps
        self.num_return_seq = config.num_return_seq
    ...
    def inference(self, enc_input, enc_input_ext, enc_pad_mask, enc_len,
        ...
        # finished hypotheses (those that have emitted the [STOP] token)
        results = []
        ...
        for steps in range(self.config.data.tgt_max_test):
            ...
            # [K x E]
            dec_emb = self.embedding(dec_input)
            h = torch.cat([hyp.hidden_state for hyp in hyps],
                          dim=0)            # [1 x H] -> [K x H]
            c = torch.cat([hyp.cell_state for hyp in hyps],
                          dim=0)              # [1 x H] -> [K x H]
            # [1 x L] -> [K x L]
            coverages = torch.cat([hyp.coverage for hyp in hyps], dim=0)
            # [1 x L x 2H] -> [K x L x 2H]
            enc_hiddens = torch.cat([enc_hidden for _ in hyps], dim=0)
            ...
            extra_zeros = torch.zeros((batch_size, max_oov_len),
                                      device=vocab_dist.device)
            extended_vocab_dist = torch.cat(
                [vocab_dist, extra_zeros], dim=-1)  # [K x V_x]
            final_dist = extended_vocab_dist.scatter_add(dim=-1,
                                                         index=enc_input_ext,
                                                         src=weighted_attn_dist)
        ...
        hyp_words = [self.vocab.outputids2words(
            hyp.tokens, src_oovs[0]) for hyp in best_hyps]
...
class PoGenSeq2Seq4ASTE(nn.Module):
    def __init__(self, embedding_matrix, config, vocab):
        super().__init__()
        self.config = config
        self.vocab = vocab
    ...
    def training_step(self, batch, batch_idx):
        ...
        output = self.model.forward(enc_input=batch['enc_input'].to(self.config.device),
                                    enc_input_ext=batch['enc_input_ext'].to(
                                        self.config.device),
                                    enc_pad_mask=batch['enc_pad_mask'].to(
                                        self.config.device),
                                    enc_len=batch['enc_len'],
                                    dec_input=batch['dec_input'].to(
                                        self.config.device),
        ...
        if self.cov_weight > 0:
            print({'train/cov_loss': cov_loss}, self.num_step)
            # self.logger.log_metrics(
            #     {'train/cov_loss': cov_loss}, self.num_step)
        self.num_step += 1
        # import pdb
        # pdb.set_trace()
        return loss
    ...
    def validation_step(self, batch, batch_idx):
        ...
        result = self.test_step(p, batch_idx)
